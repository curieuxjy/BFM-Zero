# BFM-Zero 코드베이스 개요

## 프로젝트 소개

BFM-Zero (Behavioral Foundation Model)는 비지도 강화학습을 사용한 휴머노이드 로봇 제어를 위한 행동 기반 모델입니다.

### 핵심 기능
1. **Motion Tracking**: 전문가 모션 데이터를 따라가는 정책 학습
2. **Goal Reaching**: 목표 자세로 이동하는 정책
3. **Reward-based Tasks**: 보상 함수 기반 태스크 수행

## 로봇 플랫폼: Unitree G1 (29DOF)

이 프로젝트는 **Unitree G1 휴머노이드 로봇**의 29자유도 모델을 사용합니다.

### 관절 구성 (29DOF)

```
               ┌──────────┐
               │   Head   │ (고정)
               └────┬─────┘
          ┌─────────┼─────────┐
     L Shoulder(3)  │    R Shoulder(3)
     L Elbow(1)  Waist(3)  R Elbow(1)
     L Wrist(3)     │     R Wrist(3)
     [L Hand]       │     [R Hand]     ← 패시브 (0DOF)
               ┌────┴────┐
          L Hip(3)    R Hip(3)
          L Knee(1)   R Knee(1)
          L Ankle(2)  R Ankle(2)
```

### 관절별 상세 스펙

**다리 (좌/우 × 6 = 12DOF)**

| # | 관절 | 축 | Range (rad) | Range (deg) | 토크 (N·m) |
|---|------|----|-------------|-------------|------------|
| 0 | hip_pitch | Y | -2.53 ~ 2.88 | -145° ~ 165° | ±88 |
| 1 | hip_roll | X | -0.52 ~ 2.97 | -30° ~ 170° | ±88 |
| 2 | hip_yaw | Z | -2.76 ~ 2.76 | -158° ~ 158° | ±88 |
| 3 | knee | Y | -0.09 ~ 2.88 | -5° ~ 165° | ±139 |
| 4 | ankle_pitch | Y | -0.87 ~ 0.52 | -50° ~ 30° | ±50 |
| 5 | ankle_roll | X | -0.26 ~ 0.26 | -15° ~ 15° | ±50 |

> 주: hip_roll은 좌우 대칭 (L: -0.52~2.97, R: -2.97~0.52)

**허리 (3DOF)**

| # | 관절 | 축 | Range (rad) | Range (deg) | 토크 (N·m) |
|---|------|----|-------------|-------------|------------|
| 12 | waist_yaw | Z | -2.62 ~ 2.62 | -150° ~ 150° | ±88 |
| 13 | waist_roll | X | -0.52 ~ 0.52 | -30° ~ 30° | ±50 |
| 14 | waist_pitch | Y | -0.52 ~ 0.52 | -30° ~ 30° | ±50 |

**팔 (좌/우 × 4 = 8DOF)**

| # | 관절 | 축 | Range (rad) | Range (deg) | 토크 (N·m) |
|---|------|----|-------------|-------------|------------|
| 15 | shoulder_pitch | Y | -3.09 ~ 2.67 | -177° ~ 153° | ±25 |
| 16 | shoulder_roll | X | -1.59 ~ 2.25 | -91° ~ 129° | ±25 |
| 17 | shoulder_yaw | Z | -2.62 ~ 2.62 | -150° ~ 150° | ±25 |
| 18 | elbow | Y | -1.05 ~ 2.09 | -60° ~ 120° | ±25 |

> 주: shoulder_roll은 좌우 대칭 (L: -1.59~2.25, R: -2.25~1.59)

**손목 (좌/우 × 3 = 6DOF)**

| # | 관절 | 축 | Range (rad) | Range (deg) | 토크 (N·m) |
|---|------|----|-------------|-------------|------------|
| 19 | wrist_roll | X | -1.97 ~ 1.97 | -113° ~ 113° | ±25 |
| 20 | wrist_pitch | Y | -1.61 ~ 1.61 | -92° ~ 92° | **±5** |
| 21 | wrist_yaw | Z | -1.61 ~ 1.61 | -92° ~ 92° | **±5** |

### 손(Hand)

- **개별 손가락 제어 없음** — 손은 패시브 고무 메시(`left_rubber_hand`, `right_rubber_hand`)
- 손목 3DOF(roll/pitch/yaw)로 손 방향만 제어
- 손목 pitch/yaw 토크: **±5 N·m** (다른 관절 ±25~139 N·m 대비 매우 작음)

### 토크 등급 요약

| 등급 | 토크 | 관절 |
|------|------|------|
| 대형 | ±139 N·m | knee |
| 중형 | ±88 N·m | hip(3), waist_yaw |
| 소형 | ±50 N·m | ankle(2), waist_roll/pitch |
| 팔 | ±25 N·m | shoulder(3), elbow, wrist_roll |
| 손목 | ±5 N·m | wrist_pitch, wrist_yaw |

### qpos 구조 (36값)

```
[root_pos(3), root_quat_wxyz(4), dof_pos(29)]
```

- 쿼터니언: MuJoCo 포맷 `wxyz` (내부 포맷은 `xyzw`, 변환 필요)
- 설정 파일: `humanoidverse/config/robot/g1/g1_29dof.yaml`
- 로봇 XML: `humanoidverse/data/robots/g1/g1_29dof_mujoco.xml`

---

## 아키텍처 개요

```
┌─────────────────────────────────────────────────────────────┐
│                        train.py                              │
│                      (TrainConfig)                           │
└─────────────────────────┬───────────────────────────────────┘
                          │
          ┌───────────────┼───────────────┐
          ▼               ▼               ▼
    ┌──────────┐   ┌──────────┐   ┌──────────────┐
    │  Agent   │   │   Env    │   │   Buffer     │
    │ (FBcpr)  │   │ (Isaac)  │   │ (Trajectory) │
    └────┬─────┘   └────┬─────┘   └──────────────┘
         │              │
         ▼              ▼
    ┌──────────┐   ┌──────────┐
    │  Model   │   │Simulator │
    │(nn_models)│  │(MuJoCo/  │
    └──────────┘   │ IsaacSim)│
                   └──────────┘
```

## 학습 파이프라인

1. **환경 초기화**: `HumanoidVerseIsaacConfig`로 시뮬레이션 환경 설정
2. **버퍼 생성**: 전문가 데이터 + 온라인 수집 데이터
3. **에이전트 학습**: FBcpr 알고리즘으로 정책 업데이트
4. **평가**: 모션 트래킹 EMD(Earth Mover's Distance) 측정

## 추론 파이프라인

1. **체크포인트 로드**: `load_model_from_checkpoint_dir()`
2. **환경 설정**: Hydra 오버라이드로 추론용 설정
3. **Latent z 계산**: 태스크에 따라 다른 방식으로 계산
4. **ONNX 내보내기**: 실제 로봇 배포용

## 스터디 노트 목차

| # | 파일 | 주제 |
|---|------|------|
| 01 | `01_macos_installation.md` | macOS 설치 및 문제 해결 |
| 02 | `02_train_analysis.md` | train.py 상세 분석 |
| 03 | `03_fbcpr_algorithm.md` | FB-CPR 알고리즘 (코드 중심) |
| 04 | `04_environment_config.md` | 환경 설정 시스템 |
| 05 | `05_inference_scripts.md` | 추론 스크립트 분석 |
| 06 | `06_neural_network_architecture.md` | 신경망 구조 |
| 07 | `07_learning_algorithms.md` | 학습 알고리즘 (수학/텐서 흐름) |
| 08 | `08_replay_buffers.md` | 리플레이 버퍼 |
| 09 | `09_evaluation_system.md` | 평가 시스템 (EMD) |
| 10 | `10_env_simulators.md` | 환경 & 시뮬레이터 |
| 11 | `11_motion_library.md` | 모션 라이브러리 |
| 12 | `12_inference_advanced.md` | Goal/Reward Inference 심화 |
| 13 | `13_practical_exercises.md` | 실습 가이드 & 스크립트 목록 |

## 실습 스크립트 (`study/scripts/`)

모델이 필요한 스크립트: `tracking_inference_macos.py`, `goal_inference_macos.py`, `friction_experiment.py`, `analyze_z_vectors.py`

CPU 단독 실행 가능: `debug_tensor_shapes.py`, `model_architecture_compare.py`, `mujoco_viewer_test.py`

로그/결과 파일 필요: `visualize_training.py`, `compare_reward_z.py`
