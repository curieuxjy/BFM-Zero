...
| 스크립트 | 설명 | 실행 |
|---------|------|------|
| `study/scripts/goal_inference_macos.py` | macOS용 Goal Inference (목표 프레임 → z → 롤아웃) | `uv run python study/scripts/goal_inference_macos.py --model_folder model/` |
| `study/scripts/compare_reward_z.py` | Reward Inference 결과에서 보상별 z 벡터 유사도 비교 | `uv run python study/scripts/compare_reward_z.py --result_dir model/reward_inference/` |
| `study/scripts/analyze_z_vectors.py` | 모션별 z 벡터 추출 및 코사인 유사도 분석 | `uv run python study/scripts/analyze_z_vectors.py --model_folder model/` |
| `study/scripts/12_compare_three_inferences.py` | **3가지 inference 모드 심층 비교** (z 계산 과정 시각화) | `uv run python study/scripts/12_compare_three_inferences.py --model_folder model/` |

### 12_compare_three_inferences.py 상세 사용법

```bash
# 전체 실행 (Step 1~4)
uv run python study/scripts/12_compare_three_inferences.py --model_folder model/

# Step 1만: 구조 비교 표 출력 (모델 불필요)
uv run python study/scripts/12_compare_three_inferences.py --step 1

# Step 2: Tracking z 시퀀스 분석 (프레임 간 유사도 등)
uv run python study/scripts/12_compare_three_inferences.py --model_folder model/ --step 2

# Step 3: Goal z 분석 (velocity=0 효과 비교)
uv run python study/scripts/12_compare_three_inferences.py --model_folder model/ --step 3

# Step 4: 3가지 모드의 z를 같은 모션/프레임으로 비교
uv run python study/scripts/12_compare_three_inferences.py --model_folder model/ --step 4

# 다른 모션으로 실행
uv run python study/scripts/12_compare_three_inferences.py --model_folder model/ --motion_id 0 --frame_idx 50
```
